...
<div id="dlg_insert" class="easyui-dialog" title="Add New" data-options="closed: true,modal:true" style="width: 400px; padding:10px; top: 20px;">
    <form id="frm_insert" method="post" novalidate>
        <fieldset style="width:100%; border:1px solid #d0d0d0; margin-bottom: 10px; border-radius:4px; float: left;">
            ...
            <div class="fitem">
                <span style="width:35%; display:inline-block;">Technique Name</span>
...
<script>
    ...
    function add() {
        ...
        url_save = '<?= base_url('lnd/assesment_technique/create_data') ?>';
        method = 'POST';
    ...
    function update() {
        ...
        if (row) {
            ...
            url_save = '<?= base_url('lnd/assesment_technique/update_data/') ?>' + row.id;
            method = 'PUT';
        } else {
            ...
            $.messager.confirm('Warning', 'Are you sure you want to delete this data?', function(r) {
                if (r) {
                    for (var i = 0; i < rows.length; i++) {
                        var row = rows[i];
                        fetch('<?= base_url('lnd/assesment_technique/delete_data/') ?>'+row.id, {
    ...
    $(function() {
        ...
        $('#dg').datagrid({
            url: '<?= base_url('lnd/assesment_technique/datatables') ?>',
            columns: [[
                {field: 'ck', rowspan:'2', checkbox: true},
                {field: 'assesmentId', rowspan:'2', width:150, title:'Assesment ID', halign: 'center'},
                {field: 'techniqueName', rowspan:'2', width:150, title:'Assesment Technique', halign: 'center'},
