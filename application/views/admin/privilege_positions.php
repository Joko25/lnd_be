...
<script>
    $(function() {
        $('#username').combogrid({
            onSelect: function(value, row) {
                ...
                $.ajax({
                    url: '<?= base_url('admin/privilege_positions/create') ?>',
                    type: 'post',
                    data: 'username=' + username,
                    success: function(msg) {
                        $('#dg').treegrid({
                            url: '<?= base_url('admin/privilege_positions/datatables') ?>?username=' + username,
                            rownumbers: true,
                            singleSelect: true,
                            idField: 'id',
                            treeField: 'name',
        ...
        //Get Username
        $('#username').combogrid({
            url: '<?= base_url('admin/privilege_positions/getUsers') ?>',
            panelWidth: 400,
            idField: 'username',
            textField: 'name',
            mode: 'remote',
    ...
    function save_data(data) {
        ...
        $.ajax({
            url: '<?= base_url('admin/privilege_positions/update/') ?>?id=' + window.btoa(id),
            type: 'post',
