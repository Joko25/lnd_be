...
<div id="dlg_insert" class="easyui-dialog" title="Add New" data-options="closed: true,modal:true" style="width: 400px; padding:10px; top: 20px;">
    <form id="frm_insert" method="post" novalidate>
        <fieldset style="width:100%; border:1px solid #d0d0d0; margin-bottom: 10px; border-radius:4px; float: left;">
            ...
            <div class="fitem">
                <span style="width:35%; display:inline-block;">Departement</span>
                <input style="width:60%;" name="departement_id" id="departement_id" required="" class="easyui-combogrid">
            </div>
            <div class="fitem">
...
<!-- PDF -->
<iframe id="printout" src="<?= base_url('employee/departement_subs/print') ?>" style="width: 100%;" hidden></iframe>

<script>
    //ADD DATA
    function add() {
        $('#dlg_insert').dialog('open');
        url_save = '<?= base_url('employee/departement_subs/create') ?>';
    ...
    function update() {
        ...
        if (row) {
            ...
            url_save = '<?= base_url('employee/departement_subs/update') ?>?id=' + btoa(row.id);
        } else {
            ...
            $.messager.confirm('Warning', 'Are you sure you want to delete this data?', function(r) {
                if (r) {
                    for (var i = 0; i < rows.length; i++) {
                        ...
                        $.ajax({
                            method: 'post',
                            url: '<?= base_url('employee/departement_subs/delete') ?>',
                            data: {
                                id: row.id
                            },
    ...
    function filter() {
        ...
        var filter_departement_subs = $("#filter_departement_subs").combogrid('getValue');

        var url = "?filter_divisions=" + filter_divisions + "&filter_departements=" + filter_departements + "&filter_departement_subs=" + filter_departement_subs;

        $('#dg').datagrid({
            url: '<?= base_url('employee/departement_subs/datatables') ?>' + url
        });

        $("#printout").contents().find('html').html("<center><br><br><br><b style='font-size:20px;'>Please Wait...</b></center>");
        $("#printout").attr('src', '<?= base_url('employee/departement_subs/print') ?>' + url);
    }
    ...
    function excel() {
        ...
        window.location.assign('<?= base_url('employee/departement_subs/print/excel') ?>' + url);
    }
    //RELOAD
    function reload() {
    ...
    $(function() {
        ...
        $('#dg').datagrid({
            url: '<?= base_url('employee/departement_subs/datatables') ?>',
            pagination: true,
            rownumbers: true
        });
        ...
        $('#filter_divisions').combogrid({
            ...
            onSelect: function(value, division) {
                $('#filter_departements').combogrid({
                    ...
                    onSelect: function(value, departement) {
                        $('#filter_departement_subs').combogrid({
                            url: '<?= base_url('employee/departement_subs/reads?division_id=') ?>' + division.id + '&departement_id=' + departement.id,
                            panelWidth: 420,
                            idField: 'id',
                            textField: 'name',
                            mode: 'remote',
                            ...
                            columns: [
                                [{
                                    field: 'number',
                                    title: 'Departement Sub No',
                                    width: 100
                                }, {
                                    field: 'name',
                                    title: 'Departement Sub Name',
                                    width: 250
                                }, ]
                            ],
