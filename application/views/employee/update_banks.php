...
<!-- PDF -->
<iframe id="printout" src="<?= base_url('employee/update_banks/print') ?>" style="width: 100%;" hidden></iframe>

<script>
    //ADD DATA
    // function add() {
    //     $('#dlg_insert').dialog('open');
    //     url_save = '<?= base_url('employee/update_banks/create') ?>';
    ...
    function update() {
        ...
        if (row) {
            ...
            url_save = '<?= base_url('employee/update_banks/update') ?>?id=' + btoa(row.id);
        } else {
            ...
            $.messager.confirm('Warning', 'Are you sure you want to delete this data?', function(r) {
                if (r) {
                    for (var i = 0; i < rows.length; i++) {
                        ...
                        $.ajax({
                            method: 'post',
                            url: '<?= base_url('employee/update_banks/delete') ?>',
                            data: {
                                id: row.id
                            },
    ...
    function filter() {
        ...
        $('#dg').datagrid({
            url: '<?= base_url('employee/update_banks/datatables') ?>' + url
        });

        $("#printout").contents().find('html').html("<center><br><br><br><b style='font-size:20px;'>Please Wait...</b></center>");
        $("#printout").attr('src', '<?= base_url('employee/update_banks/print') ?>' + url);
    }
    ...
    function excel() {
        ...
        window.location.assign('<?= base_url('employee/update_banks/print/excel') ?>' + url);
    }
    //RELOAD
    function reload() {
    ...
    $(function() {
        ...
        $('#dg').datagrid({
            url: '<?= base_url('employee/update_banks/datatables') ?>',
            pagination: true,
            rownumbers: true
        });
        ...
        $('#dlg_upload').dialog({
            buttons: [{
                text: 'List Failed',
                handler: function() {
                    window.open('<?= base_url('employee/update_banks/uploadDownloadFailed') ?>', '_blank');
                }
            }, {
                text: 'Upload',
                iconCls: 'icon-ok',
                handler: function() {
                    $('#frm_upload').form('submit', {
                        url: '<?= base_url('employee/update_banks/upload') ?>',
                        onSubmit: function() {
                        ...
                        success: function(result) {
                            ...
                            //Clear File
                            $.ajax({
                                url: "<?= base_url('employee/update_banks/uploadclearFailed') ?>"
                            });
                            ...
                            function requestData(total, json, number = 1, value = 0, success = 1, failed = 1) {
                                if (value < 100) {
                                    ...
                                    $.ajax({
                                        type: "POST",
                                        async: true,
                                        url: "<?= base_url('employee/update_banks/uploadCreate') ?>",
                                        data: {
                                            "data": json[number - 1]
                                        },
                                        cache: false,
                                        ...
                                        success: function(result) {
                                            if (result.theme == "success") {
                                                ...
                                                $.ajax({
                                                    type: "POST",
                                                    async: true,
                                                    url: "<?= base_url('employee/update_banks/uploadcreateFailed') ?>",
                                                    data: {
                                                        data: json[number - 1],
