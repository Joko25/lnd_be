...
<table id="dg" class="easyui-datagrid" style="width:99.5%;" toolbar="#toolbar">
    <thead>
        <tr>
            ...
            <th rowspan="2" data-options="field:'amount',width:100,halign:'center',formatter:numberformat">Amount</th>
...
<div id="toolbar" style="height: 225px;">
    <fieldset style="width: 99%; border:2px solid #d0d0d0; margin-bottom: 5px; margin-top: 5px; border-radius:4px;">
        ...
        <div style="width: 50%; float: left;">
            ...
            <div class="fitem">
                <span style="width:30%; display:inline-block;">Correction Type</span>
                <select style="width:60%;" id="filter_correction" panelHeight="auto" class="easyui-combobox">
    ...
    <form id="frm_insert" method="post" novalidate>
        <fieldset style="width:100%; border:1px solid #d0d0d0; margin-bottom: 10px; border-radius:4px; float: left;">
            ...
            <div class="fitem">
                ...
                <input style="width:60%;" name="trans_date" id="trans_date" required="" class="easyui-datebox" required data-options="formatter:myformatter,parser:myparser, editable:false">
            </div>
            ...
            <div class="fitem" style="margin-bottom: 10px;">
                <span style="width:35%; display:inline-block;">Correction Type</span>
                <input name="correction_type" id="correction_type" value="PLUS" checked class="easyui-radiobutton"> Plus &nbsp;
                <input name="correction_type" value="MINUS" class="easyui-radiobutton"> Minus
            </div>
            <div class="fitem">
                <span style="width:35%; display:inline-block;">Correction Name</span>
            ...
            <div class="fitem">
                ...
                <input style="width:60%;" name="amount" id="amount" required="" class="easyui-numberbox">
            </div>
            <div class="fitem">
...
<!-- PDF -->
<iframe id="printout" src="<?= base_url('payroll/corrections/print') ?>" style="width: 100%;" hidden></iframe>

<script>
    //ADD DATA
    function add() {
        $('#dlg_insert').dialog('open');
        url_save = '<?= base_url('payroll/corrections/create') ?>';
        $('#frm_insert').form('clear');
        $('#employee_id').combogrid('enable');
        $('#trans_date').datebox('setValue', "<?= date("Y-m-d") ?>");
        $('#correction_type').radiobutton({
            checked: true
        });
    }
    ...
    function update() {
        ...
        if (row) {
            ...
            url_save = '<?= base_url('payroll/corrections/update') ?>?id=' + window.btoa(row.id);
        } else {
            ...
            $.messager.confirm('Warning', 'Are you sure you want to delete this data?', function(r) {
                if (r) {
                    for (var i = 0; i < rows.length; i++) {
                        ...
                        $.ajax({
                            method: 'post',
                            url: '<?= base_url('payroll/corrections/delete') ?>',
                            data: {
                                id: row.id
                            },
    ...
    function download_excel() {
        window.location.assign('<?= base_url('template/tmp_correction.xls') ?>');
    }
    ...
    function filter() {
        ...
        var filter_correction = $("#filter_correction").combobox('getValue');
        ...
        $("#printout").attr('src', '<?= base_url('payroll/corrections/print') ?>' + url);
    }
    ...
    function excel() {
        ...
        window.location.assign('<?= base_url('payroll/corrections/print/excel') ?>' + url);
    }
    //RELOAD
    function reload() {
    ...
    $(function() {
        ...
        $('#dlg_upload').dialog({
            buttons: [{
                text: 'List Failed',
                handler: function() {
                    window.open('<?= base_url('payroll/corrections/uploadDownloadFailed') ?>', '_blank');
                }
            }, {
                text: 'Upload',
                iconCls: 'icon-ok',
                handler: function() {
                    $('#frm_upload').form('submit', {
                        url: '<?= base_url('payroll/corrections/upload') ?>',
                        onSubmit: function() {
                        ...
                        success: function(result) {
                            ...
                            //Clear File
                            $.ajax({
                                url: "<?= base_url('payroll/corrections/uploadclearFailed') ?>"
                            });
                            ...
                            function requestData(total, json, number = 1, value = 0, success = 1, failed = 1) {
                                if (value < 100) {
                                    ...
                                    $.ajax({
                                        type: "POST",
                                        async: true,
                                        url: "<?= base_url('payroll/corrections/uploadCreate') ?>",
                                        data: {
                                            "data": json[number - 1]
                                        },
                                        cache: false,
                                        ...
                                        success: function(result) {
                                            if (result.theme == "success") {
                                                ...
                                                $.ajax({
                                                    type: "POST",
                                                    async: true,
                                                    url: "<?= base_url('payroll/corrections/uploadcreateFailed') ?>",
                                                    data: {
                                                        data: json[number - 1],
