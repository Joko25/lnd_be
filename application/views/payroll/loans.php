...
<table id="dg" class="easyui-datagrid" style="width:99.5%;" toolbar="#toolbar">
    <thead>
        <tr>
            ...
            <th rowspan="2" data-options="field:'trans_date',width:120,halign:'center'">Trans Date</th>
            ...
            <th rowspan="2" data-options="field:'loan_type',width:100,halign:'center'">Type</th>
...
<div id="dlg_insert" class="easyui-dialog" title="Add New" data-options="closed: true,modal:true" style="width: 400px; padding:10px; top: 20px;">
    <form id="frm_insert" method="post" novalidate>
        <fieldset style="width:100%; border:1px solid #d0d0d0; margin-bottom: 10px; border-radius:4px; float: left;">
            ...
            <div class="fitem">
                <span style="width:30%; display:inline-block;">Loan Type</span>
                <select style="width:60%;" id="loan_type" name="loan_type" required="" panelHeight="auto" class="easyui-combobox">
            ...
            <div class="fitem">
                <span style="width:30%; display:inline-block;">Amount</span>
...
<!-- PDF -->
<iframe id="printout" src="<?= base_url('payroll/loans/print') ?>" style="width: 100%;" hidden></iframe>

<script>
    //ADD DATA
    function add() {
        $('#dlg_insert').dialog('open');
        url_save = '<?= base_url('payroll/loans/create') ?>';
        $('#frm_insert').form('clear');
        $('#employee_id').combogrid('enable');
        $('#trans_date').textbox('setValue', "<?= date("Y-m-d") ?>");
    }
    ...
    function update() {
        ...
        if (row) {
            ...
            url_save = '<?= base_url('payroll/loans/update') ?>?id=' + window.btoa(row.id);
        } else {
            ...
            $.messager.confirm('Warning', 'Are you sure you want to delete this data?', function(r) {
                if (r) {
                    for (var i = 0; i < rows.length; i++) {
                        ...
                        $.ajax({
                            method: 'post',
                            url: '<?= base_url('payroll/loans/delete') ?>',
                            data: {
                                id: row.id
                            },
    ...
    function download_excel() {
        window.location.assign('<?= base_url('template/tmp_loan.xls') ?>');
    }
    ...
    function filter() {
        ...
        $('#dg').datagrid({
            url: '<?= base_url('payroll/loans/datatables') ?>' + url
        });

        $("#printout").contents().find('html').html("<center><br><br><br><b style='font-size:20px;'>Please Wait...</b></center>");
        $("#printout").attr('src', '<?= base_url('payroll/loans/print') ?>' + url);
    }
    ...
    function excel() {
        ...
        window.location.assign('<?= base_url('payroll/loans/print/excel') ?>' + url);
    }
    //RELOAD
    function reload() {
    ...
    $(function() {
        $('#dg').datagrid({
            url: '<?= base_url('payroll/loans/datatables') ?>',
            pagination: true,
            rownumbers: true
        });
        ...
        $('#dlg_upload').dialog({
            buttons: [{
                text: 'List Failed',
                handler: function() {
                    window.open('<?= base_url('payroll/loans/uploadDownloadFailed') ?>', '_blank');
                }
            }, {
                text: 'Upload',
                iconCls: 'icon-ok',
                handler: function() {
                    $('#frm_upload').form('submit', {
                        url: '<?= base_url('payroll/loans/upload') ?>',
                        onSubmit: function() {
                        ...
                        success: function(result) {
                            ...
                            //Clear File
                            $.ajax({
                                url: "<?= base_url('payroll/loans/uploadclearFailed') ?>"
                            });
                            ...
                            function requestData(total, json, number = 1, value = 0, success = 1, failed = 1) {
                                if (value < 100) {
                                    ...
                                    $.ajax({
                                        type: "POST",
                                        async: true,
                                        url: "<?= base_url('payroll/loans/uploadCreate') ?>",
                                        data: {
                                            "data": json[number - 1]
                                        },
                                        cache: false,
                                        ...
                                        success: function(result) {
                                            if (result.theme == "success") {
                                                ...
                                                $.ajax({
                                                    type: "POST",
                                                    async: true,
                                                    url: "<?= base_url('payroll/loans/uploadcreateFailed') ?>",
                                                    data: {
                                                        data: json[number - 1],
