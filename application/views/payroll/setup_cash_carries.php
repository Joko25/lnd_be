        ...
        <tr>
            ...
            <th rowspan="2" data-options="field:'allowance_name',width:200,halign:'center', editor:{
                ...
                options:{
                    valueField:'name',
                    textField:'name',
                    url:'<?= base_url('payroll/allowance_cash_carries/reads') ?>',
                    required:true
                }
            }">Allowance</th>
...
<!-- PDF -->
<iframe id="printout" src="<?= base_url('payroll/setup_cash_carries/print') ?>" style="width: 100%;" hidden></iframe>

<script>
    //ADD DATA
    function add() {
        $('#dlg_insert').dialog('open');
        url_save = '<?= base_url('payroll/setup_cash_carries/create') ?>';
    ...
    function update() {
        ...
        if (rows.length == 1) {
            for (var i = 0; i < rows.length; i++) {
                ...
                url_save = '<?= base_url('payroll/setup_cash_carries/update') ?>?id=' + btoa(row.id);
            }
        } else {
                ...
                if (r) {
                    for (var i = 0; i < rows.length; i++) {
                        ...
                        if ($.inArray(row, checked) >= 0) {
                            $.ajax({
                                method: 'post',
                                url: '<?= base_url('payroll/setup_cash_carries/delete') ?>',
                                data: {
                                    id: row.id
                                },
    ...
    function download_excel() {
        window.location.assign('<?= base_url('template/tmp_setup_cash_carries.xls') ?>');
    }
    ...
    function filter() {
        ...
        $('#dg').datagrid({
            url: '<?= base_url('payroll/setup_cash_carries/datatables') ?>' + url
        });

        $("#printout").contents().find('html').html("<center><br><br><br><b style='font-size:20px;'>Please Wait...</b></center>");
        $("#printout").attr('src', '<?= base_url('payroll/setup_cash_carries/print') ?>' + url);
    }
    ...
    function excel() {
        ...
        window.location.assign('<?= base_url('payroll/setup_cash_carries/print/excel') ?>' + url);
    }
    //RELOAD
    function reload() {
    ...
    $(function() {
        ...
        $('#dg').datagrid({
            ...
            onEndEdit: function(index, row) {
                ...
                $.ajax({
                    type: "post",
                    url: "<?= base_url('payroll/setup_cash_carries/createOrUpdate') ?>",
                    data: "employee_id=" + employee_id + "&allowance_name=" + allowance_name,
                    dataType: "json",
        ...
        $('#dlg_upload').dialog({
            buttons: [{
                text: 'List Failed',
                handler: function() {
                    window.open('<?= base_url('payroll/setup_cash_carries/uploadDownloadFailed') ?>', '_blank');
                }
            }, {
                text: 'Upload',
                iconCls: 'icon-ok',
                handler: function() {
                    $('#frm_upload').form('submit', {
                        url: '<?= base_url('payroll/setup_cash_carries/upload') ?>',
                        onSubmit: function() {
                        ...
                        success: function(result) {
                            ...
                            //Clear File
                            $.ajax({
                                url: "<?= base_url('payroll/setup_cash_carries/uploadclearFailed') ?>"
                            });
                            ...
                            function requestData(total, json, number = 1, value = 0, success = 1, failed = 1) {
                                if (value < 100) {
                                    ...
                                    $.ajax({
                                        type: "POST",
                                        async: true,
                                        url: "<?= base_url('payroll/setup_cash_carries/uploadCreate') ?>",
                                        data: {
                                            "data": json[number - 1]
                                        },
                                        cache: false,
                                        ...
                                        success: function(result) {
                                            if (result.theme == "success") {
                                                ...
                                                $.ajax({
                                                    type: "POST",
                                                    async: true,
                                                    url: "<?= base_url('payroll/setup_cash_carries/uploadcreateFailed') ?>",
                                                    data: {
                                                        data: json[number - 1],
