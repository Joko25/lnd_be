...
<table id="dg" class="easyui-datagrid" style="width:99.5%;" toolbar="#toolbar">
    <thead>
        <tr>
            ...
            <th rowspan="2" data-options="field:'employee_name',width:150,halign:'center'">Employee</th>
...
<div id="toolbar" style="height: 200px; padding:10px;">
    ...
    <?= $button ?>
    <a href="javascript:;" class="easyui-linkbutton" data-options="plain:true" onclick="unregistered()"><i class="fa fa-users"></i> Unregistered Shift</a>
</div>
    ...
    <form id="frm_insert" method="post" novalidate>
        <fieldset style="width:100%; border:1px solid #d0d0d0; margin-bottom: 10px; border-radius:4px; float: left;">
            ...
            <div class="fitem">
                ...
                <input style="width:60%;" name="shift_id" id="shift_id" required class="easyui-combobox" panelHeight="auto">
            </div>
        </fieldset>
    </form>
</div>
...
<div id="dlg_unregistered" class="easyui-dialog" title="Unregistered Shift" data-options="closed: true,modal:true" style="width: 500px; height: 500px; top: 20px;">
...
<!-- PDF -->
<iframe id="printout" src="<?= base_url('attandance/shift_employees/print') ?>" style="width: 100%;" hidden></iframe>

<script>
    //ADD DATA
    function add() {
        $('#dlg_insert').dialog('open');
        url_save = '<?= base_url('attandance/shift_employees/create') ?>';
    ...
    function update() {
        ...
        if (row) {
            ...
            url_save = '<?= base_url('attandance/shift_employees/update') ?>?id=' + btoa(row.id);
        } else {
            ...
            $.messager.confirm('Warning', 'Are you sure you want to delete this data?', function(r) {
                if (r) {
                    for (var i = 0; i < rows.length; i++) {
                        ...
                        $.ajax({
                            method: 'post',
                            url: '<?= base_url('attandance/shift_employees/delete') ?>',
                            data: {
                                id: row.id
                            },
    ...
    function download_excel() {
        window.location.assign('<?= base_url('template/tmp_shift_employees.xls') ?>');
    }
    ...
    function filter() {
        ...
        $("#printout").attr('src', '<?= base_url('attandance/shift_employees/print') ?>' + url);
    }
    ...
    function unregistered() {
        ...
        $('#dg_unregistered').datagrid({
            url: '<?= base_url('attandance/shift_employees/readUnregistered') ?>',
            clientPaging: false,
            remoteFilter: true,
            rownumbers: true
    ...
    $(function() {
        ...
        $('#dg').datagrid({
            url: '<?= base_url('attandance/shift_employees/datatables') ?>',
            pagination: true,
            rownumbers: true
        });
        ...
        $('#dlg_upload').dialog({
            buttons: [{
                text: 'List Failed',
                handler: function() {
                    window.open('<?= base_url('attandance/shift_employees/uploadDownloadFailed') ?>', '_blank');
                }
            }, {
                text: 'Upload',
                iconCls: 'icon-ok',
                handler: function() {
                    $('#frm_upload').form('submit', {
                        url: '<?= base_url('attandance/shift_employees/upload') ?>',
                        onSubmit: function() {
                        ...
                        success: function(result) {
                            ...
                            //Clear File
                            $.ajax({
                                url: "<?= base_url('attandance/shift_employees/uploadclearFailed') ?>"
                            });
                            ...
                            function requestData(total, json, number = 1, value = 0, success = 1, failed = 1) {
                                if (value < 100) {
                                    ...
                                    $.ajax({
                                        type: "POST",
                                        async: true,
                                        url: "<?= base_url('attandance/shift_employees/uploadCreate') ?>",
                                        data: {
                                            "data": json[number - 1]
                                        },
                                        cache: false,
                                        ...
                                        success: function(result) {
                                            if (result.theme == "success") {
                                                ...
                                                $.ajax({
                                                    type: "POST",
                                                    async: true,
                                                    url: "<?= base_url('attandance/shift_employees/uploadcreateFailed') ?>",
                                                    data: {
                                                        data: json[number - 1],
