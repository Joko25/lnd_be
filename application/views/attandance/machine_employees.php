...
<table id="dg" class="easyui-datagrid" style="width:100%;" toolbar="#toolbar">
    <thead>
        <tr>
            ...
            <th rowspan="2" data-options="field:'machine_code',width:100,halign:'center'">ID Machine</th>
    ...
    <form id="frm_insert" method="post" novalidate>
        <fieldset style="width:100%; border:1px solid #d0d0d0; margin-bottom: 10px; border-radius:4px; float: left;">
            ...
            <div class="fitem">
                ...
                <input style="width:60%;" name="number" disabled class="easyui-textbox">
            </div>
            <div class="fitem">
            ...
            <div class="fitem">
                <span style="width:35%; display:inline-block;">ID Machine</span>
                <input style="width:60%;" name="machine_code" required="" class="easyui-textbox">
            </div>
        </fieldset>
    </form>
</div>
...
<!-- PDF -->
<iframe id="printout" src="<?= base_url('attandance/machine_employees/print') ?>" style="width: 100%;" hidden></iframe>

<script>
    //ADD DATA
    // function add() {
    //     $('#dlg_insert').dialog('open');
    //     url_save = '<?= base_url('attandance/machine_employees/create') ?>';
    ...
    function update() {
        ...
        if (row) {
            ...
            url_save = '<?= base_url('attandance/machine_employees/update') ?>?id=' + btoa(row.id);
        } else {
    ...
    function download_excel() {
        window.location.assign('<?= base_url('template/tmp_machine_employee.xls') ?>');
    }
    ...
    function filter() {
        ...
        $('#dg').datagrid({
            url: '<?= base_url('attandance/machine_employees/datatables') ?>' + url
        });

        $("#printout").contents().find('html').html("<center><br><br><br><b style='font-size:20px;'>Please Wait...</b></center>");
        $("#printout").attr('src', '<?= base_url('attandance/machine_employees/print') ?>' + url);
    }
    ...
    function excel() {
        ...
        window.location.assign('<?= base_url('attandance/machine_employees/print/excel') ?>' + url);
    }
    //RELOAD
    function reload() {
    ...
    $(function() {
        ...
        $('#dg').datagrid({
            url: '<?= base_url('attandance/machine_employees/datatables') ?>',
            pagination: true,
            rownumbers: true,
            fit: true,
        ...
        $('#dlg_upload').dialog({
            buttons: [{
                text: 'List Failed',
                handler: function() {
                    window.open('<?= base_url('attandance/machine_employees/uploadDownloadFailed') ?>', '_blank');
                }
            }, {
                text: 'Upload',
                iconCls: 'icon-ok',
                handler: function() {
                    $('#frm_upload').form('submit', {
                        url: '<?= base_url('attandance/machine_employees/upload') ?>',
                        onSubmit: function() {
                        ...
                        success: function(result) {
                            ...
                            //Clear File
                            $.ajax({
                                url: "<?= base_url('attandance/machine_employees/uploadclearFailed') ?>"
                            });
                            ...
                            function requestData(total, json, number = 1, value = 0, success = 1, failed = 1) {
                                if (value < 100) {
                                    ...
                                    $.ajax({
                                        type: "POST",
                                        async: true,
                                        url: "<?= base_url('attandance/machine_employees/uploadCreate') ?>",
                                        data: {
                                            "data": json[number - 1]
                                        },
                                        cache: false,
                                        ...
                                        success: function(result) {
                                            if (result.theme == "success") {
                                                ...
                                                $.ajax({
                                                    type: "POST",
                                                    async: true,
                                                    url: "<?= base_url('attandance/machine_employees/uploadcreateFailed') ?>",
                                                    data: {
                                                        data: json[number - 1],
