...

{
...

    {
        ...
        $data = $this->crud->query("SELECT b.id, b.name, COUNT(a.number) as total FROM maritals b LEFT JOIN employees a ON a.marital_id = b.id AND a.status = 0 GROUP BY b.id, b.name ORDER BY b.name ASC");
        $categories = array();
        $values = array();
        $total = 0;
        foreach ($data as $row) {
