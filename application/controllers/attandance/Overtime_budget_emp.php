...
class Overtime_budget_emp extends CI_Controller
{
    public function __construct()
    {
        ...
        //VALIDASI FORM
        $this->form_validation->set_rules('employee_id', 'Employee', 'required|min_length[1]|max_length[30]|is_unique[overtime_budgets.employee_id]');
    }
    ...
    public function index()
    {
        if (empty($this->session->username)) {
            ...
            $this->load->view('attandance/overtime_budget_emp');
        } else {
    ...
    public function datatables()
    {
        if ($this->input->post()) {
            ...
            $result = array();
            //Select Query
            $this->db->select('a.*, b.number as employee_number, b.name as employee_name');
            $this->db->from('overtime_budgets a');
            $this->db->join('employees b', 'a.employee_id = b.id');
            $this->db->where('a.deleted', 0);
            $this->db->where('a.type', 'EMP');
    ...
    {
        @unlink('failed/overtime_budget_emp.txt');
    }
    ...
    public function uploadcreateFailed()
    {
        if ($this->input->post()) {
            ...
            $textFailed = fopen('failed/overtime_budget_emp.txt', 'a');
    ...
    {
        $file = "failed/overtime_budget_emp.txt";
    ...
    public function uploadcreate()
    {
        if ($this->input->post()) {
            ...
            $overtime_budget = $this->crud->read('overtime_budgets', [], ["employee_id" => @$employee->id]);
                ...
                <table style="width: 100%;">
                    <tr>
                        ...
                        <td style="font-size: 14px; text-align: left; margin:2px;">
                            <b>' . $config->name . '</b><br>
                            <small>MASTER OVERTIME BUDGET EMPLOYEE</small>
                        </td>
                    </tr>
                </table>
            </div>
