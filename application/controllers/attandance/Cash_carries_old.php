...

{
    ...
    public function readOvertimePrice($employee_id, $trans_date, $duration, $meal, $remarks = "")

    {
        ...
        // Cek apakah ada keyword HOLIDAY di remarks/deskripsi

        $is_holiday_keyword = false;

        if (!empty($remarks) && stripos($remarks, 'HOLIDAY') !== false) {

            $is_holiday_keyword = true;

        }



        // Prioritas tertinggi: Jika ada keyword HOLIDAY di remarks, gunakan tarif holiday

        if ($is_holiday_keyword) {

            $total = ((@$allowance_cash_carry->holiday * $duration) + $meal);

        } else if (@$shift_employee->days == "5") {
    ...
    // Fungsi baru untuk menghitung plan amount berdasarkan keyword HOLIDAY

    public function calculatePlanAmount($employee_id, $trans_date, $duration, $meal, $remarks = "")
    ...
    // Endpoint untuk menghitung plan amount berdasarkan keyword HOLIDAY

    public function calculatePlanAmountAjax()

    {
        ...
        if ($this->input->post()) {
            ...
            $plan_amount = $this->calculatePlanAmount($employee_id, $trans_date, $duration, $meal, $remarks);
            ...
            echo json_encode(array(
                ...
                "message" => "Plan amount calculated successfully"
...

    {
        ...
        if ($this->input->post()) {
            ...
            foreach ($records as $record) {
                ...
                // Jika kedua time_in dan time_out kosong, set durasi dan amount ke 0
                if (empty($attandance->time_in) && empty($attandance->time_out)) {
                    $att_hour = 0;
                    $hour = 0;
                    $meal = 0;
                    $total = 0;
                } else {
                    // Jika salah satu ada, gunakan logika yang sudah ada
                    if (!empty($attandance->time_in) && !empty($attandance->time_out)) {
                        ...
                        $att_time_begin = strtotime($record['trans_date'] . " " . $record['start']);
                        $att_time_end = strtotime($record['trans_date'] . " " . $record['end']);
                    }
                
                if (!empty($attandance->time_out)) {
                    $att_time_end = strtotime(@$attandance->date_out . " " . @$attandance->time_out);
                } else {
                    // Ambil dari cash_carries end jika attendance kosong
                    $att_time_end = strtotime($record['trans_date'] . " " . $record['end']);
                }

                if (!empty($attandance->time_in)) {
                    $att_time_begin = strtotime(@$attandance->date_in . " " . @$attandance->time_in);
                } else {
                    // Ambil dari cash_carries start jika attendance kosong
                    $att_time_begin = strtotime($record['trans_date'] . " " . $record['start']);
                }
                ...
                // Cek apakah ada keyword HOLIDAY di remarks/deskripsi untuk Actual Amount
                $is_holiday_keyword = false;
                if (!empty($record['remarks']) && stripos($record['remarks'], 'HOLIDAY') !== false) {
                    $is_holiday_keyword = true;
                }
...

    {
        ...
        if ($this->input->post()) {
            ...
            $ot_amount = $this->readOvertimePrice($employee_id, $trans_date, $duration['duration_hour'], $meal, $remarks);



            // Jika plan kosong atau 0, hitung berdasarkan keyword HOLIDAY di remarks

            if (empty($plan) || $plan == 0) {

                $plan = $this->calculatePlanAmount($employee_id, $trans_date, $duration['duration_hour'], $meal, $remarks);
...

    {
        ...
        if ($this->input->post()) {
            ...
            // Menambahkan validasi format tanggal (YYYY-MM-DD)
            ...
            if ($data['trans_date'] == "" || !isValidDate($data['trans_date'])) {
                ...
                if (!empty($employee)) {
                    ...
                    if (!empty($cash_carries)) {
                        ...
                        $ot_amount = $this->readOvertimePrice($employee->id, $data['trans_date'], $duration['duration_hour'], $meal, $data['remarks']);



                        // Jika plan kosong atau 0, hitung berdasarkan keyword HOLIDAY di remarks

                        if (empty($data['plan']) || $data['plan'] == 0) {

                            $data['plan'] = $this->calculatePlanAmount($employee->id, $data['trans_date'], $duration['duration_hour'], $meal, $data['remarks']);
...

    {
        ...
        // Calculate amount_actual for each record like in datatables
