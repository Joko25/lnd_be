...
class Departements extends CI_Controller
{
    public function __construct()
    {
        ...
        //VALIDASI FORM
        $this->form_validation->set_rules('number', 'Code', 'required|min_length[1]|max_length[20]|is_unique[departements.number]');
    }
    ...
    public function index()
    {
        if (empty($this->session->username)) {
            ...
            $this->load->view('employee/departements');
        } else {
    ...
    public function create()
    {
        if ($this->input->post()) {
            if ($this->form_validation->run() == TRUE) {
                ...
                $send   = $this->crud->create('departements', $post);
                echo $send;
            } else {
    ...
    public function update()
    {
        if ($this->input->post()) {
            ...
            $post = $this->input->post();
            $send = $this->crud->update('departements', ["id" => $id], $post);
            echo $send;
        } else {
            show_error("Cannot Process your request");
        }
    ...
    public function delete()
    {
        ...
        $send = $this->crud->delete('departements', $data);
        echo $send;
    }
    ...
    public function print($option = "")
    {
        if ($option == "excel") {
            ...
            header("Content-Disposition: attachment; filename=departements_$format.xls");
        }
