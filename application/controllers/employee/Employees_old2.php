...
class Employees extends CI_Controller
{
    ...
    //UPDATE STATUS EMPLOYEE YANG SUDAH LEWAT TANGGAL INACTIVE BERDASARKAN RESIGN DATE
    // Logika baru:
    // 1. Jika resign_date masuk dalam range periode payroll (cutoff), maka inactive tgl 30 bulan resign
    // 2. Jika resign_date di luar range periode payroll, maka inactive tgl 30 bulan M+1 resign
    private function updateResignedEmployeeStatus()
    {
        $today = date("Y-m-d");
        
        // Ambil periode cutoff terbaru
        $latestCutoff = $this->getLatestCutoffPeriod();
        
        if (!$latestCutoff) {
            log_message('warning', 'No cutoff period found for employee status update');
            return;
        }
        
        // Ambil semua employee yang memiliki resign_date
        $this->db->select('a.id, a.number, a.name, i.resign_date');
        $this->db->from('employees a');
        $this->db->join('resignations i', 'a.id = i.employee_id', 'left');
        $this->db->where('i.resign_date IS NOT NULL');
        $this->db->where('a.status', 0); // Hanya yang masih active
        $resignedEmployees = $this->db->get()->result_array();
        
        foreach ($resignedEmployees as $employee) {
            if ($employee['resign_date']) {
                // Tentukan tanggal inactive berdasarkan logika baru
                $inactiveDate = $this->calculateInactiveDate($employee['resign_date'], $latestCutoff->start, $latestCutoff->finish);
                
                // Jika hari ini sudah lewat tanggal inactive, update status menjadi inactive
                if ($today > $inactiveDate->format('Y-m-d')) {
                    $this->crud->update('employees', 
                        ["id" => $employee['id']], 
                        ["status" => 1, "status_date" => $inactiveDate->format('Y-m-d')]
                    );
                    
                    // Log untuk tracking
                    log_message('info', 'Employee ' . $employee['number'] . ' (' . $employee['name'] . ') status changed to inactive. Resign date: ' . $employee['resign_date'] . ', Cutoff period: ' . $latestCutoff->start . ' to ' . $latestCutoff->finish . ', Inactive date: ' . $inactiveDate->format('Y-m-d'));
                }
            }
        }
    }

    //CALCULATE INACTIVE DATE BASED ON RESIGN DATE AND CUTOFF PERIOD
    private function calculateInactiveDate($resignDate, $cutoffStart, $cutoffFinish)
    {
        $resignDateObj = new DateTime($resignDate);
        $cutoffStartObj = new DateTime($cutoffStart);
        $cutoffFinishObj = new DateTime($cutoffFinish);
        
        // Cek apakah resign date masuk dalam range periode payroll (cutoff)
        if ($resignDateObj >= $cutoffStartObj && $resignDateObj <= $cutoffFinishObj) {
            // Jika resign date masuk dalam range periode payroll, inactive tgl 30 bulan resign
            $inactiveDate = clone $resignDateObj;
            $inactiveDate->modify('last day of this month');
        } else {
            // Jika resign date di luar range periode payroll, inactive tgl 30 bulan M+1 resign
            $inactiveDate = clone $resignDateObj;
            $inactiveDate->modify('first day of next month');
            $inactiveDate->modify('last day of this month');
        }
    ...
    //PUBLIC METHOD TO GET INACTIVE DATE - CAN BE USED BY OTHER CONTROLLERS
    public function getInactiveDateByResignDate($resignDate)
    {
        $latestCutoff = $this->getLatestCutoffPeriod();
        ...
        $inactiveDate = $this->calculateInactiveDate($resignDate, $latestCutoff->start, $latestCutoff->finish);
        return $inactiveDate->format('Y-m-d');
    }
    ...
    {
        // Auto update status employee yang sudah lewat akhir bulan resign date
        $this->updateResignedEmployeeStatus();
    ...
    public function print_employee_detail($number)
    {
        ...
        $this->db->select('a.*, b.name as division_to, c.name as departement_to, d.name as departement_sub_to, a.description');
        ...
        $mutations = $this->db->get()->result();

        // Ambil data posisi awal karyawan
        ...
        // Gabungkan data: posisi awal + mutasi
        $result = [];
        
        // Tambahkan semua mutasi (dari terbaru ke lama)
        foreach ($mutations as $mutation) {
            $result[] = (array) $mutation;
        }
        
        // Tambahkan posisi awal sebagai mutasi terakhir (jika ada)
        if ($initial_position) {
