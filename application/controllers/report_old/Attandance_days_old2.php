...
class Attandance_days extends CI_Controller
{
    ...
    public function printcsv(){
        if ($this->input->get()) {
            ...
            foreach ($records as $data) {
                ...
                $this->db->where("resign_date <= '$filter_to'");
                $resignation = $this->db->get()->row();

                //Change Day Start - Pindahkan ke sini sebelum query kehadiran
                $this->db->select("*");
                ...
                //Attandance and Overtime - CSV Export
                $this->db->select("a.date_in, a.time_in, a.date_out, a.time_out");
                $this->db->from('attandances a');
                // Ambil data kehadiran di range filter dan juga tanggal-tanggal start dari change_days
                $whereDate = "a.date_in between '$filter_from' and '$filter_to'";
                if (count($allChangeDayStart) > 0) {
                    $escapedDates = array_map(function($d) { return "'" . addslashes($d) . "'"; }, $allChangeDayStart);
                    $whereDate .= " OR a.date_in IN (" . implode(",", $escapedDates) . ")";
                }
                $this->db->where("($whereDate)");
                $this->db->where('a.number', $data['employee_number']);
                
                // Filter berdasarkan waktu masuk sesuai shift
                if (!empty($filter_shift)) {
                    if ($filter_shift == "1") {
                        // Shift 1: 05:00-09:00
                        $this->db->where("a.time_in >= '05:00:00' AND a.time_in <= '09:00:00'");
                    } elseif ($filter_shift == "2") {
                        // Shift 2: 15:00-22:00
                        $this->db->where("a.time_in >= '15:00:00' AND a.time_in <= '22:00:00'");
                    } elseif ($filter_shift == "3") {
                        // Shift 3: 22:30-00:00 (melewati tengah malam)
                        $this->db->where("(a.time_in >= '22:30:00' OR a.time_in <= '00:00:00')");
                    }
                }
                ...
                //Change Day Start - Hapus bagian ini karena sudah dipindah ke atas
                // $this->db->select("*");
                // $this->db->from('change_days');
                // $this->db->where('employee_id', $data['employee_id']);
                // $this->db->where("(start between '$filter_from' and '$filter_to' or end between '$filter_from' and '$filter_to')");
                    ...
                    if ($showThisDay) {
                        ...
                        if ($showAfterResign) {
                            ...
                            $data2 = array(
                                ...
                                !empty($resign_date) ? date("d-m-Y", strtotime($resign_date)) : '',
    ...
    public function print($option = "")
    {
        ...
        if ($this->input->get()) {
            ...
            foreach ($records as $data) {
                // Ambil resign date terakhir (max) untuk karyawan ini
                $this->db->select_max('resign_date');
                $this->db->from('resignations');
                $this->db->where('employee_id', $data['employee_id']);
                $resign_row = $this->db->get()->row();
                $resign_date = !empty($resign_row->resign_date) ? $resign_row->resign_date : '';
                //Shifts
                $this->db->select("d.start, d.end, d.days, d.working, d.tolerance, c.name, d.name as shift_name");
                $this->db->from('shift_employees b');
                $this->db->join('shifts c', 'c.id = b.shift_id');
                ...
                //Attandance and Overtime - Print Export
                $this->db->select("a.date_in, a.time_in, a.date_out, a.time_out");
                $this->db->from('attandances a');
                // Ambil data kehadiran di range filter dan juga tanggal-tanggal start dari change_days
                $whereDate = "a.date_in between '$filter_from' and '$filter_to'";
                ...
                //Shifts - Hapus bagian ini karena sudah ada di atas
                // $this->db->select("d.start, d.end, d.days, d.working, d.tolerance, c.name, d.name as shift_name");
                // $this->db->from('shift_employees b');
                // $this->db->join('shifts c', 'c.id = b.shift_id');
                // $this->db->join('shift_details d', 'd.shift_id = c.id');
                ...
                //PERMIT - Hapus bagian ini karena sudah ada di atas
                // $this->db->select("a.*, c.name as reason_name, d.name as permit_name");
                // $this->db->from('permits a');
                // $this->db->join('reasons c', 'a.reason_id = c.id');
                // $this->db->join('permit_types d', 'a.permit_type_id = d.id');
                ...
                //Resignation - Hapus bagian ini karena sudah ada di atas
                // $this->db->select('*');
                // $this->db->from('resignations');
                // $this->db->where('employee_id', $data['employee_id']);
                    ...
                    if ($showThisDay) {
                        ...
                        if ($showAfterResign) {
                            ...
                            $html =     '<tr>
                                            ...
                                            <td>' . (!empty($resign_date) ? date("d-m-Y", strtotime($resign_date)) : '') . '</td>
