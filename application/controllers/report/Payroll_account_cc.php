...
class Payroll_account_cc extends CI_Controller
{
    ...
    public function index()
    {
        if (empty($this->session->username)) {
            ...
            $this->load->view('report/payroll_account_cc');
        } else {
    ...
    public function print($option = "")
    {
        if ($option == "excel") {
            ...
            header("Content-Disposition: attachment; filename=payroll_account_cc_$format.xls");
        }
        ...
        if ($this->input->get()) {
            ...
            $html = '<html><head><title>Print Data</title></head><style>body {font-family: Arial, Helvetica, sans-serif;}#customers {border-collapse: collapse;width: '.$width.'%;font-size: 10px;}#customers td, #customers th {border: 1px solid #ddd;padding: 2px;}#customers tr:nth-child(even){background-color: #f2f2f2;}#customers tr:hover {background-color: #ddd;}#customers th {padding-top: 2px;padding-bottom: 2px;text-align: left;color: black;}</style>
                    ...
                    <center>
                        <h3 style="margin:0;">Payroll Account Cash Carry</h3>
...
//----------------------------------------------------- Detail By Division Karyawan
        ...
        foreach ($divisions as $division) {
            ...
            foreach ($jobtypes as $jobtype) {
                $accounts = $this->crud->query("SELECT DISTINCT b.number as account_no, b.id as account_id FROM account_coa a JOIN accounts b ON a.account_id = b.id WHERE a.division_id = '$division->id' and a.job_type = '$jobtype->job_type' and b.category = 'cash carry'");
                ...
                foreach ($accounts as $account) {
                    ...
                    JOIN accounts g ON a.account_id = g.id and g.category = 'cash carry'
                    ...
                    foreach ($records as $record) {
                        ...
                        while (strtotime($start) <= strtotime($end)) {
                            ...
                            FROM account_coa a
                            JOIN accounts b ON a.account_id = b.id and b.category = 'cash carry'
                            ...
                            LEFT JOIN cash_carries i ON h.id = i.employee_id and DATE_FORMAT(STR_TO_DATE(i.trans_date, '%Y-%m'), '%Y-%m') = '$start'
                    ...
                    while (strtotime($start2) <= strtotime($end2)) {
                        ...
                        JOIN accounts b ON a.account_id = b.id and b.category = 'cash carry'
                        LEFT JOIN employees h ON a.division_id = h.division_id and a.departement_id = h.departement_id and a.position_id = h.position_id and a.contract_id = h.contract_id
                        LEFT JOIN cash_carries i ON h.id = i.employee_id and DATE_FORMAT(STR_TO_DATE(i.trans_date, '%Y-%m'), '%Y-%m') = '$start2'
        ...
        while (strtotime($start3) <= strtotime($end3)) {
            ...
            JOIN accounts b ON a.account_id = b.id and b.category = 'cash carry'
            LEFT JOIN employees h ON a.division_id = h.division_id and a.departement_id = h.departement_id and a.position_id = h.position_id and a.contract_id = h.contract_id
            LEFT JOIN cash_carries i ON h.id = i.employee_id and DATE_FORMAT(STR_TO_DATE(i.trans_date, '%Y-%m'), '%Y-%m') = '$start3'");
        ...
        $html .= '<tr>
            ...
            foreach ($summaries as $summary) {
                ...
                while (strtotime($sumstart) <= strtotime($sumend)) {
                    ...
                    JOIN accounts b ON a.account_id = b.id and b.category = 'cash carry'
                    LEFT JOIN employees h ON a.division_id = h.division_id and a.departement_id = h.departement_id and a.position_id = h.position_id and a.contract_id = h.contract_id
                    LEFT JOIN cash_carries i ON h.id = i.employee_id and DATE_FORMAT(STR_TO_DATE(i.trans_date, '%Y-%m'), '%Y-%m') = '$sumstart'
            ...
            while (strtotime($start4) <= strtotime($end4)) {
                $employeeGrand = $this->crud->query("SELECT
                    ...
                    JOIN accounts b ON a.account_id = b.id and b.category = 'cash carry'
                    LEFT JOIN employees h ON a.division_id = h.division_id and a.departement_id = h.departement_id and a.position_id = h.position_id and a.contract_id = h.contract_id
                    LEFT JOIN cash_carries i ON h.id = i.employee_id and DATE_FORMAT(STR_TO_DATE(i.trans_date, '%Y-%m'), '%Y-%m') = '$start4'");
