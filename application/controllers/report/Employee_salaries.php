...

{
...

    {
        ...
        if ($this->input->get()) {
            ...
            $query = $this->db->query("SELECT b.bank_no, a.net_income, a.number, a.name, d.name as source_name FROM (
                SELECT employee_id, net_income, number, name, period_start, period_end FROM payrolls 
                WHERE period_start = '$period_start' AND period_end = '$period_end'
                UNION ALL
                SELECT employee_id, net_income, number, name, period_start, period_end FROM payroll_harian_lepas 
                WHERE period_start = '$period_start' AND period_end = '$period_end'
            ) a
...

    {
        ...
        if ($filter_bank == "Bank Central Asia") {
            ...
            $query = $this->db->query("SELECT b.bank_no, a.net_income, a.number, a.name FROM (
                SELECT employee_id, net_income, number, name, period_start, period_end FROM payrolls 
                WHERE period_start = '$period_start' AND period_end = '$period_end'
                UNION ALL
                SELECT employee_id, net_income, number, name, period_start, period_end FROM payroll_harian_lepas 
            ...
            $query = $this->db->query("SELECT a.number, a.name, b.bank_no, a.net_income FROM (
                SELECT employee_id, net_income, number, name, period_start, period_end FROM payrolls 
                WHERE period_start = '$period_start' AND period_end = '$period_end'
                UNION ALL
                SELECT employee_id, net_income, number, name, period_start, period_end FROM payroll_harian_lepas 
