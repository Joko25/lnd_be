...
class Attandances extends CI_Controller
{
    ...
    public function readCalendar()
    {
        ...
        for ($i = $start; $i <= $finish; $i += (60 * 60 * 24)) {
            ...
            $records[] = array(
                ...
                "overtime_request" => @$attandance->request_code,
    ...
    public function create()
    {
        if ($this->input->post()) {
            ...
            $attandaces = $this->crud->reads('attandance_generates', [], ["employee_id" => $post['employee_id'], "date_in" => $post['date_in']]);

            if (count($attandaces) > 0) {
                $send = $this->crud->update('attandance_generates', ["employee_id" => $post['employee_id'], "date_in" => $post['date_in']], $post);

                if ($send) {
                    echo json_encode(array("title" => "Success", "message" => date("d F Y", strtotime($post['date_in'])) . " has been updated", "theme" => "success"));
                } else {
                    echo json_encode(array("title" => "Error", "message" => date("d F Y", strtotime($post['date_in'])) . " update failed", "theme" => "error"));
                }
            } else {
                $send = $this->crud->create('attandance_generates', $post);

                if ($send) {
                    echo json_encode(array("title" => "Success", "message" => date("d F Y", strtotime($post['date_in'])) . " has been created", "theme" => "success"));
                } else {
                    echo json_encode(array("title" => "Error", "message" => date("d F Y", strtotime($post['date_in'])) . " create failed", "theme" => "error"));
                }
            }
        } else {
