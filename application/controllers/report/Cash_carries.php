        ...
        if ($this->input->get()) {
            ...
            foreach ($employee_data as $emp) {
                $html .= '  <tr>
                                ...
                                <td style="text-align:right;">' . number_format($emp['total_amount'], 0, ',', '.') . '</td>
            ...
            $html .= '  <tr>
                        <th style="text-align:right;" colspan="8">GRAND TOTAL</th>
                        <th style="text-align:right;">' . number_format($grandTotalAmount, 0, ',', '.') . '</th>
    ...
    public function print_detail($option = "")
    {
        ...
        if ($this->input->get()) {
            ...
            $html = '<html><head><title>Print Data</title></head>
                     <style>body {font-family: Arial, Helvetica, sans-serif;} #customers {border-collapse: collapse; width: 100%; font-size: 10px;} #customers td, #customers th {border: 1px solid #000; padding: 4px;} #customers th {text-align: center; background-color: #f2f2f2;} .str{mso-number-format:"\@";} .num{mso-number-format:"[$-421]#,##0";} .num2{mso-number-format:"#,##0.00";} .date{mso-number-format:"dd-mmm";}</style>
                     <body>';
            $html .= '<h3 style="text-align:center;">REPORT DETAIL CASH CARRY</h3>
                      <p style="text-align:center; font-size:12px;">Period ' . date("d F Y", strtotime($filter_from)) . ' to ' . date("d F Y", strtotime($filter_to)) . '</p><br>';
